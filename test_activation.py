...
class TestActivation(TestCase):
    def test__tanh(self):
        ...
        # Compare vectors
        vect = np.array([1, 2, 3])
        true_values = np.array([0.7615941559557649, 0.9640275800758169, 0.9950547536867305])
        self.assertTrue(np.array_equal(activation.function(vect), true_values))

        # Compare matrices
        matrix = np.array([[0.2, 0.0000001, 3], [-0.5, 1000, -10]])
        true_values = np.array([[0.19737532, 0.0000001, 0.99505475], [-0.46211716, 1., -1.]])
        self.assertTrue(np.array_equal(np.round(activation.function(matrix), 8), true_values))
        ...
        true_values = np.array([0.41997434161402614, 0.07065082485316443, 0.009866037165440211])
